            ...
            <el-form :model="saveForm" label-width="80px" :rules="saveFormRules" ref="saveForm">
                ...
                <el-form-item label="类型">
                    <el-select v-model="saveForm.typeId">
                        <el-option :value="item.id" :label="item.name" v-for="item in sellerTypes"/>
                    </el-select>
                </el-form-item>
            </el-form>
...
export default {
    ...
    methods: {
        ...
        getSeller() {
            ...
            this.$http.post("/seller", this.query)
                .then(data => {
        ...
        getSellerTypes() {
            this.$http.get("/sellerType")
                .then(result=>{
                    result = result.data;
                    if (result.success) {
                        this.sellerTypes = result.resultObject;
                    }
                })
        },
        //删除
        handleDel: function (index, row) {
            this.$confirm('确认删除该记录吗?', '提示', {
                ...
                this.listLoading = true;
                this.$http.delete("/seller/" + row.id).then((data) => {
                    this.listLoading = false;
                    data = data.data;
                    if (data.success) {
                        this.$message({
            ...
            this.$refs.saveForm.validate((valid) => {
                if (valid) {
                    this.$confirm('确认提交吗？', '提示', {}).then(() => {
                        ...
                        this.$http.put("/seller", this.saveForm).then((data) => {
