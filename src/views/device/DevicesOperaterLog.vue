...
export default {
    ...
    methods: {
        ...
        //获取列表
        getDevicesOperaterLog() {
            this.listLoading = true;
            this.$http.post("/devicesOperaterLog", this.query)
                .then(data => {
        ...
        getDevicesOperaterLogTypes() {
            this.$http.get("/devicesOperaterLogType")
                .then(result=>{
                    result = result.data;
                    if (result.success) {
                        this.devicesOperaterLogTypes = result.resultObject;
                    }
                })
        },
        //删除
        handleDel: function (index, row) {
            this.$confirm('确认删除该记录吗?', '提示', {
                ...
                this.listLoading = true;
                this.$http.delete("/devicesOperaterLog/" + row.id).then((data) => {
                    this.listLoading = false;
                    data = data.data;
                    if (data.success) {
                        this.$message({
        ...
        handleEdit: function (index, row) {
            ...
            this.getDevicesOperaterLogTypes()
        },
        //显示新增界面
        handleAdd: function () {
            this.saveFormVisible = true;
            ...
            this.$refs.saveForm.validate((valid) => {
                if (valid) {
                    this.$confirm('确认提交吗？', '提示', {}).then(() => {
                        ...
                        this.$http.put("/devicesOperaterLog", this.saveForm).then((data) => {
        ...
        batchRemove: function () {
            ...
            this.$confirm('确认删除选中记录吗？', '提示', {
                ...
                this.listLoading = true;
                this.$http.patch("/devicesOperaterLog", ids)
                    .then((data) => {
                        this.listLoading = false;
                        data = data.data;
                        if (data.success) {
