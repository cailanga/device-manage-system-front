    ...
    <el-dialog title="领用" :visible.sync="saveFormVisible" :close-on-click-modal="false">
      <el-form :model="saveForm" label-width="80px" :rules="saveFormRules" ref="saveForm">
        ...
        <el-form-item label="保质期(月)">
          <el-input disabled v-model="saveForm.qualityguaranteeperiod" auto-complete="off" type="number"
                    :min="0"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    setUseDept(row){
      ...
      this.getDeptTree();
      this.getDeptsByDeviceId(row.id);
    },
    //获取设备可领用部门信息
    getDeptsByDeviceId(deviceId){
      this.$http.get("/device/byDevice/"+deviceId)
          .then(data=>{
            data = data.data
    ...
    saveSubmit1: function () {
      ...
      this.$confirm('确认提交吗？', '提示', {}).then(() => {
        ...
        // 存放设备和可领用部门的对应关系
        let deviceDept = {
          deviceId:this.saveForm1.deviceId,
          deviceDepts:[]
        }
        if (parent!=null&&parent.length > 0) {
          for (let item of parent) {
            ...
            deviceDept.deviceDepts.push({
              deptId: deptId,
              deptPath : deptPath,
            })
          }
        }
        this.$http.post("/device/setCanUseDept",deviceDept)
            .then(data=>{
              data = data.data
              if (data.success){
                this.$message({
