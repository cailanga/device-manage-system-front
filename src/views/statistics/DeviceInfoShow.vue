      ...
      this.$http.post('/deviceShow/device', this.query3)
          .then(data => {
            ...
            if(this.pageInfo3.total>0){
              this.config.data = this.pageInfo3.rows
              this.chart3TableDataUpdate(this.config.data,1)
              //动态折线更新
      ...
      this.$http.post('/deviceShow/devicePriceChange/' + deviceId)
          .then(data => {
            ...
            if (this.myChart3 && this.myChart3.series && this.myChart3.series[0]) {
              ...
              console.log("Series not defined in myChart3");
            }
          })
          .catch(error => {
            console.error('Error fetching data: ', error);
      ...
      this.$http.post('/deviceShow/countsWithType', this.query)
          .then(data => {
            ...
            if(this.pageInfo.total>0){
              this.dataFromBackend = this.pageInfo.rows[0];
              this.dataFromBackend1 = this.pageInfo.rows[1]
            }else {
              this.dataFromBackend = []
              this.dataFromBackend1 = []
            }
            this.initChart();
          })
          .catch(error => {
            ...
            itemStyle: {
              normal: {
                color: function (params) {
                  ...
                  return colorList[(params.dataIndex+1)%colorList.length]
                },
                shadowBlur: 200,
                shadowColor: 'rgba(24,200,120, 0.5)'
              }
